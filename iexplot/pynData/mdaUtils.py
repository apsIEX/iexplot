#==============================================================================
# Data processing for APS MDA files
# 2020-02-28
# 
# Ver. 1
...
def readMDA(fname, verbose=0):
    ...
    if len(dim)==1:
        ...
        # Outer loop
        slowPosKey = 'P1: {}'.format(mdata[1].p[0].name.decode("utf-8"))
        slowPos = np.array(mdata[1].p[0].data)

        # Inner loop
        fastPosKey = 'P2: {}'.format(mdata[2].p[0].name.decode("utf-8"))
        fastPos = np.array(mdata[2].p[0].data)[0]

        fastPosMesh, slowPosMesh = np.meshgrid(fastPos, slowPos)
        outPV[slowPosKey] = slowPosMesh
        outPV[fastPosKey] = fastPosMesh
