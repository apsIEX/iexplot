...
# version 3.0 Tim Mooney 4/23/2020
...
def readScan(scanFile, verbose=0, out=sys.stdout, unpacker=None):
	...
	scan.name = u.unpack_string().decode('utf-8')

	if verbose: out.write("scan.name = %s\n" % (scan.name))

	timelength = u.unpack_int()

	scan.time = u.unpack_string().decode('utf-8')
	...
	for j in range(scan.nd):
		...
		if length: scan.d[j].name = u.unpack_string().decode('utf-8')

		if verbose: out.write("scan.d[%d].name = %s\n" % (j, scan.d[j].name))

		length = u.unpack_int() # length of desc string

		if length: scan.d[j].desc = u.unpack_string().decode('utf-8')

		if verbose: out.write("scan.d[%d].desc = %s\n" % (j, scan.d[j].desc))

		length = u.unpack_int() # length of unit string

		if length: scan.d[j].unit = u.unpack_string().decode('utf-8')
	...
	for j in range(scan.nt):
		...
		if length: scan.t[j].name = u.unpack_string().decode('utf-8')
...
def readMDA(fname=None, maxdim=4, verbose=0, showHelp=0, outFile=None, useNumpy=None, readQuick=False):
	...
	if pExtra:
		...
		for i in range(numExtra):
			...
			if n: name = u.unpack_string().decode('utf-8')
			...
			if n: desc = u.unpack_string().decode('utf-8')
			...
			if EPICS_type != 0:   # not DBR_STRING; array is permitted
				...
				if n: unit = u.unpack_string().decode('utf-8')
			...
			if EPICS_type == 0: # DBR_STRING
				...
				if n: value = u.unpack_string().decode('utf-8')
